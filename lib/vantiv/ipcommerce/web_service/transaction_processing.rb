...
module Ipcommerce
	class Web_service
		module Transaction_processing
			...
			REST_SCHEMA=":http:\/\/schemas.ipcommerce.com\/CWS\/v2.0\/Transactions\/Rest" if !defined?(REST_SCHEMA)
        ...
        request[:__type] = "AuthorizeAndCaptureTransaction"+REST_SCHEMA
        request[:ApplicationProfileId] = @application_id
        request[:MerchantProfileId] = merchant_profile_id
        request[:Transaction] = ActiveSupport::OrderedHash.new
        request[:Transaction][:__type] = "BankcardTransaction:http:\/\/schemas.ipcommerce.com\/CWS\/v2.0\/Transactions\/Bankcard"
        transaction[:TransactionData][:TransactionDateTime]=iso_time(transaction[:TransactionData][:TransactionDateTime])
				request[:Transaction].merge!(transaction)
#				request=
#        {
...
#				request[:Transaction].update(transaction)
        ...
        request = ActiveSupport::OrderedHash.new
        request[:__type] = "AuthorizeTransaction"+REST_SCHEMA
        request[:ApplicationProfileId] = @application_id
        request[:MerchantProfileId] = merchant_profile_id
        request[:Transaction] = ActiveSupport::OrderedHash.new
        request[:Transaction][:__type] = "BankcardTransaction:http:\/\/schemas.ipcommerce.com\/CWS\/v2.0\/Transactions\/Bankcard"
        ...
        request = ActiveSupport::OrderedHash.new
        request[:__type] = "Undo:http:\/\/schemas.ipcommerce.com\/CWS\/v2.0\/Transactions\/Rest"
        request[:ApplicationProfileId] = @application_id

        request[:DifferenceData] = ActiveSupport::OrderedHash.new
        request[:DifferenceData][:__type] ="BankcardUndo:http:\/\/schemas.ipcommerce.com\/CWS\/v2.0\/Transactions\/Bankcard"
        request[:DifferenceData][:TransactionId] = transaction_id
				request[:DifferenceData][:Addendum] = nil
				request[:DifferenceData][:PINDebitReason] = 0
				request[:DifferenceData][:TenderData] = nil
